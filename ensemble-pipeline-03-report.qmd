...
these_clusterings <- coi[, weights$instance] %>% align_classes(weights$instance[1])
lv <- levels(these_clusterings[[weights$instance[1]]])
ps <- apply(these_clusterings, 1, function(u) table(factor(u, levels=lv))) %>% t
...
if (unlist(strsplit(name, "-", fixed=TRUE))[1]=="libd_dlpfc" && nclust==7){
  coi <- align_classes(coi, ref=gt, columns=c(weights$instance, consensus_names))
  
  # For DLPFC example, using the original DLPFC palette
  #              L3,        L1,        WM,        L5,         L6,        L2,        L4
  palette <- c("#4DAF4A", "#F0027F", "#1A1A1A", "#FFD700", "#FF7F00", "#377EB8", "#984EA3")
}
...
# ggsave("libd_con.pdf", con_p, height=4, width=2.7)

# Entropy plot
ent_p <- lapply(paste0("ent_", c(consensus_names, "bc")), function(ent){

  ace_q <- ggplot(coi, aes(row, col, colour=.data[[ent]])) +
    geom_point(size = pt_size-0.2) +
...
#ggsave("libd_label.pdf", gt_p, height=4, width=3.5)
```
...
bc_p <- lapply(c(weights$instance), function(col_p){
  ...
  base_p <- ggplot(coi, aes(row, col, colour=as.factor(.data[[col_p]]))) + 
    ...
    scale_y_continuous(expand = expansion(0,1)) +
    ggtitle(title_text) +
      #paste(col_p, round(weights[col_p, "weights"], 3), sep="-")) 
    coord_fixed()
...
# keepers <- names(bases)
aris_all <- calc_aris(coi[,c(as.character(weights$instance), gt)])
aris_all <- aris_all - diag(nrow = nrow(aris_all))
ph_all <- pheatmap(aris_all, 
                   ...
                   color = viridisLite::viridis(50))

ggsave("ari_libd.pdf", ph_all[[4]], width = 4, height = 4)
show(ph_all)
```
...
pdf(sprintf("report_%s_nclust_%s_nmeth_%s_0412.pdf", name, nclust, nmethod), width=12, height=7)
grid.arrange(grobs = c(con_p, gt_p[1], ent_p), #gt_p[length(gt_p)]), 
                      ncol=length(consensus_names)+1, top=sprintf("Cross-method entropy for different consensus methods. Cluster number: %s", nclust))
if (spot_agreement){
  grid.arrange(grobs = c(con_p, gt_p[1], sg_p, gt_p[length(gt_p)]), 
               ncol=length(consensus_names)+1, top=sprintf("Consensus spot-wise agreement compared against %s.", gt[1]))
  grid.arrange(grobs = c(con_p, gt_p[1], anti_sg_p, gt_p[length(gt_p)]), 
               ncol=length(consensus_names)+1, top="Consensus spot-wise agreement (inversed) with ground truth labels.")
}
