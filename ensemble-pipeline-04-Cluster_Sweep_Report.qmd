---
title: |
  Consensus clustering sweep report \
...
nclust_list <- params$nclust_list
# # for testing
# name <- "thalamus-6800"
...
df <- read.delim(file.path(datadir, paste0("df_consensus_", name, "_", nmethod, ".tsv")), stringsAsFactors = FALSE, row.names=1,
                 colClasses=c("character", rep(NA, ncol(read.delim(file.path(datadir, paste0("df_consensus_", name, "_", nmethod, ".tsv")), header=TRUE))-1)))

length_vec <- apply(df, 2, function(x) length(unique(x)))
length_vec <- length_vec[length_vec < 50]
```

```{r plot the consensus label first, fig.width=6, fig.height=6, eval=TRUE}
for (nclust in nclust_list){
  con_names <- paste0("consensus_", m_con, "_", nclust)
  if (all(length_vec[con_names] ==  length_vec[con_names[1]])){
    df <- align_classes(df,
                        ref=con_names, 
                        columns=con_names[1])
  }
...
palette <- distinctColorPalette(k=max(length_vec))

plot_spatial <- function(label, data, palette, pt_size=pt_size){
  this_palette <- palette[1:length(unique(data[[label]]))] %>% setNames(unique(data[[label]]))
...
if ("lca" %in% m_con){
  p_lca <- lapply(paste0("consensus_lca_", nclust_list), plot_spatial, data=df, palette=palette, pt_size=pt_size)
}
if ("kmodes" %in% m_con){
  p_kmodes <- lapply(paste0("consensus_kmodes_", nclust_list), plot_spatial, data=df, palette=palette, pt_size=pt_size)
}
if ("weighted" %in% m_con){
  p_weighted <- lapply(paste0("consensus_weighted_", nclust_list), plot_spatial, data=df, palette=palette, pt_size=pt_size)
}

p_gt <- lapply(gt, plot_spatial, data=df, palette=palette, pt_size=pt_size)

```


```{r quick plot of entropy}
for (nclust in nclust_list){
  weights <- read.delim(file.path(datadir, paste0("weights_consensus_", name, "_nmeth_", nmethod, "_nclust_", nclust, ".tsv")), stringsAsFactors = FALSE, row.names=1)

  # Use this as a reference
  lca_con <- sprintf("consensus_lca_%s", nclust)
  # Get only the base clusterings and the target consensus
  these_clusterings <- df[, c(weights$instance, lca_con)] %>% 
                       align_classes(ref=lca_con)

  # Extracting the clustering level. Need to share the same label as the rest
  lv <- levels(these_clusterings[[lca_con]])

  # remove the consensus before calculating
  these_clusterings[[lca_con]] <- NULL
  ...
  df[[sprintf("ent_%s", nclust)]] <- apply(ps, 1, calc_entropy)
}

p_ent <- lapply(paste0("ent_", nclust_list), function(ent){
    ace_q <- ggplot(df, ) +
    geom_point(aes(row, col, colour=.data[[ent]]), size = pt_size) +
    ...
    scale_y_continuous(expand = expansion(0,1)) +
    coord_fixed() +
    labs(title = sprintf("entropy_%s", gsub("ent_", "", ent)), 
        subtitle = sprintf("mean entropy: %.4f", mean(df[[ent]])), 
        colour = "entropy") +
...
```{r load base-clustering results}
bc_list <- lapply(nclust_list, function(nclust){
  w_df <- read.delim(file.path(datadir, sprintf("weights_consensus_%s_nmeth_%s_nclust_%s.tsv", 
                               name, nmethod, nclust)), 
                     stringsAsFactors = FALSE, 
                     row.names=1)
  return(w_df)
}) %>% setNames(paste0("weights_", nclust_list))

#ARI plots
p_ARI <- lapply(bc_list, function(w_df){
  
  base_clusterings <- df %>%
  dplyr::select(c(w_df$instance, gt)) %>%
  align_classes(w_df$instance[1])

  # Calculate pair-wise ARI results
  aris_all <- calc_aris(base_clusterings)
  nclust <- strsplit(w_df$instance[1], "_") %>% unlist() %>% .[length(.)]
  ph_all <- pheatmap(aris_all, 
                    fontsize = 8, 
                    treeheight_row = 0,
                    treeheight_col = 20,
                    main=sprintf('Cross method ARI %s', nclust))

  return(ph_all[[4]])
})

# Base-clustering plots
p_bc <- lapply(bc_list, function(w_df){

  base_clusterings <- df %>%
  dplyr::select(c(w_df$instance, "row", "col")) %>%
  align_classes(ref = w_df$instance[1], columns = w_df$instance)

  bc_plots <- lapply(w_df$instance, plot_spatial, data=base_clusterings, palette=palette, pt_size=pt_size)

  return(bc_plots)
})
```

```{r plot entropy emphasize on boundary points}
# Label a point base on whether it is a boundary point
find_boundary_points <- function(data, row, col, gt, k = 5) {
  # Use FNN::get.knn to find k nearest neighbors
  nn <- dbscan::kNN(data[, c(row, col)], k = k)
  
  # Initialize a column to store if the point is a boundary
  data$boundary <- FALSE

  # Get labels of nearest neighbors for all points
  neighbor_labels <- matrix(data[[gt]][nn$id], ncol = k)

  # Compare neighbor labels with each point's label
  current_labels <- matrix(data[[gt]], nrow = nrow(data), ncol = k)
  
  # Check if any neighbor has a different label for each point (vectorized)
  data[[sprintf("border_%s", gt)]] <- rowSums(neighbor_labels != current_labels) > 0
  
  return(data)
}

# Generate the border structure
df <- find_boundary_points(df, "row", "col", gt=gt[1], k=5)

# Plot the entropy with the border structure
p_entBorder <- lapply(paste0("ent_", nclust_list), function(ent){
    ace_q <- ggplot(df, aes(row, col)) +
    geom_point(size = 1.5, shape=21, aes(fill=.data[[ent]], color=.data[[sprintf("border_%s", gt[1])]]), stroke=0.5) +
    scale_fill_gradient(low="gray90", high="deeppink4") +
    scale_color_manual(values = c("0000FF7F", "#FFA500CC")) + 
    theme_classic() +
    theme(legend.position = "none", 
        axis.title.x=element_blank(),
        axis.text.x=element_blank(),
...
```{r save a quick pdf}
pdf(sprintf("report_%s_nmeth_%s_0412.pdf", name, nmethod), width=16, height=10)
grid.arrange(grobs = c(p_lca, #p_gt[1], 
                       p_weighted, #p_gt[2],
                       p_kmodes, #p_gt[1],
                       p_entBorder #, p_gt[2]
                       ), ncol=4,
                       top = "Consensus results for 16, 20 and 24 clusters on top 8 smoothest base-clustering results")
for (i in seq_along(nclust_list)){
  grid.arrange(grobs = p_bc[[i]], ncol = 4, top = sprintf("base clusterings for cluster %s", nclust_list[i]))
}
dev.off()
# top=sprintf("Cross-method entropy for different consensus methods. Cluster number: %s", nclust)

ggsave(filename = sprintf("entropy_border_%s_nmeth_%s.pdf", name, nmethod), 
       plot = marrangeGrob(grobs = c(p_entBorder, p_gt[1]), nrow=2, ncol=2, top = "Entropy of the top 8 base-clusterings results, for cluster number 16, 20 and 24, with border marked"),
       width = 23, height = 11)

ggsave(filename = sprintf("pwARI_%s_nmeth_%s.pdf", name, nmethod), 
       plot = marrangeGrob(grobs = p_ARI, nrow=1, ncol=3, top = "pair-wise ARI of the selected base-clusterings results, for cluster number 16, 20 and 24"),
       width = 15, height = 5)
```
...
ace_q <- ggplot(df) +
  geom_point(aes(row, col, colour=ent_16), size = pt_size) +
  scale_colour_gradient(low="#E5E5E51A", high="darkgreen") +
  new_scale_color() +
  geom_point(aes(row, col, colour=ent_20), size = pt_size-0.1, alpha=0.6) +
  scale_colour_gradient(low="#E5E5E51A", high="yellow") +
  new_scale_color() +
  geom_point(aes(row, col, colour=ent_24), size = pt_size-0.2, alpha=0.3) +
  scale_colour_gradient(low="#E5E5E51A", high="darkred") +
  theme_classic() +
...
ggsave("ent_multi_thalamus.pdf", ace_q, width=6, height=3)
```
...
```{r load-weights, eval=TRUE}

weights <- read.delim(file.path(datadir, paste0("weights_consensus_", name, "_", nmethod, ".
tsv")), stringsAsFactors = FALSE, row.names=1)
consensus_names <- paste0("consensus_", m_con, "_", nclust)
...
grid.arrange(grobs = c(con_p, gt_p[1], ent_p, gt_p[length(gt_p)]), ncol=length(consensus_names)+1, top=sprintf("Cross-method entropy for different consensus methods. Cluster number: %s", nclust))
if (spot_agreement){
