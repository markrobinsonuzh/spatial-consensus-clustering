...
```{r test runtime and smoothness for LCA consensus results, fig.width=8, fig.height=6}
...
```{r form consensus via iteration methods}
...
#                      dplyr::select(names(real_n), !starts_with(c("seurat", "scanpy")))
...
colnames(binary_ensemble) <- rownames(selected_clusters)
# Ensemble labelling
ensemble_label <- get_cluster_label(binary_ensemble, n_clust_target=nclust, verbose=T)

# Save ensemble results
df$consensus_KLWEN <- ensemble_label
```

```{r visualization consensus results, fig.height=6, fig.width=10}
plot_cols <- c("consensus_lca", "consensus_KLWEN", "consensus_kmodes", gt)
...
```{r plot weight results, fig.height=6, fig.width=10}
...
aligned_bases <- align_classes(df, ref = "consensus_KLWEN", 
...
plot_list <- lapply(c("consensus_KLWEN", names(weight_vector), gt), function(label){
...
```{r save results}
write.csv(df, file.path(datadir, paste0("df_consensus_", name, ".tsv")), sep="\t")
write.csv(weight_df, file.path(datadir, paste0("weights_consensus_", name, ".tsv")), sep="\t")
```
