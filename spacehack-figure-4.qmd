...
clust <- cutree(ph_all$tree_row, k=4)
...
show_these <- anno_row %>% filter(real_n==7, clust==2) %>% 
  ...
  filter(method != "annotation") %>% head(6) %>% pull(method)
...
# show(ph_all)
save_pheatmap_pdf(ph_all, "figure-4-heatmap-all.pdf", 
                  width = 9, height=8)
...
for(meth in show_set) {
  ...
  names(palette) <- levels(df$this_base)
...
ggsave(filename = paste0("across-clusterings-entropy.pdf"),
...
save_pheatmap_pdf(ph_cents, "figure-4-heatmap-across-entropy.pdf", 
                  width = 3, height=5)
...
ggsave("figure4-aris-L10-anno.pdf", p, height = 5, width = 6)
